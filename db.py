...
class facebookListingDataRecord:
    ...
    def insert(self, data: jsonListingData):
        ...
        self.cursor.execute(f"""
            ...
            (ticker, record_hash, batch_id, listing_id, primary_listing_photo, if_gk_just_listed_tag_on_search_feed,
        ...
        """, (data.ticker, data.record_hash, str(data.batch_id), data.listing_id, json.dumps(data.primary_listing_photo),
        ...
        print(f"Inserted {data.ticker} (hash: {str(data.record_hash)[:8]}...)")
        return True
